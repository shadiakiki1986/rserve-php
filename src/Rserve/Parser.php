	...
	public static function parse($buf, &$offset) {
        ...
        break;
			...
			case self::XT_ARRAY_CPLX:
				...
				$real = array();
				while ($i < $eoa) {
					$real[] = Helpers::flt64($r, $i);
					$i += 8;
					$im[] = Helpers::flt64($r, $i);
					$i += 8;
				}
				if (count($real) == 1) {
					$a = array($real[0], $im[0]);
	...
	*/
	...
	public static function createBinary(\Rserve\REXP $value) {
		...
		switch($type) {
			...
			case self::XT_INT:
				$v = (int)$value->at(0);
				$contents .= Helpers::mkint32($v);
				$o += 4;
				break;
			case self::XT_DOUBLE:
				$v = (float)$value->at(0);
				$contents .= Helpers::mkfloat64($v);
				$o += 8;
				break;
