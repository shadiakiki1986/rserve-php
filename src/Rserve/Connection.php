...
class Connection {
	...
	public function evalString($string, $parser = self::PARSER_NATIVE) {
		...
		$data = Helpers::_rserve_make_data(self::DT_STRING, $string);
	...
	public function setEncoding($encoding) {
		$this->sendCommand(self::CMD_setEncoding, Helpers::_rserve_make_data(self::DT_STRING, $encoding));
	}
	...
	protected function sendCommand($command, $data) {
		...
		$pkt = Helpers::_rserve_make_packet($command, $data);
	...
	public function debugPacket($packet) {
		...
		*/
		...
		$command = Helpers::int32($header, 0);
		$lengthLow = Helpers::int32($header, 4);
		$offset = Helpers::int32($header, 8);
		$lenghtHigh = Helpers::int32($header, 12);
		...
		$i = 0;
		while($len > 0) {
			$type = Helpers::int8($buf, $i);
			$m_len = Helpers::int24($buf, $i+1);
			$i += 4;
			$i += $m_len;
			$len -= $m_len + 4;
